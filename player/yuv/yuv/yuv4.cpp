...
void Yuv4::getYPixel(unsigned char *buffer, int pixel, short *y, short *pb, short *pr, int max_width, int max_height)
{
	int height;
	int width = pixel % max_width;
	if (pixel == 0) {
...
void Yuv4::setYPixel(unsigned char y, int pixel, unsigned char *buf, int max_width, int max_heightf)
{
	int height;
	int width = pixel % max_width;
	if (pixel == 0) {
	...
	*(buf + height_offset + width + y_offset_cbcr + y_offset_y) = y;
