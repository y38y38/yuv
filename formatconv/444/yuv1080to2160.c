...
void copy_hd_to_4k( uint16_t*dst, int16_t *src)
{
    ...
    for (int j=0;j< src_vertical;j++) {
        ...
        //down left
        memcpy(dst + ((src_horizontal * src_vertical) * 2) + (j * dst_horizontal) , 
               src + (j * src_horizontal) , src_horizontal * 2);
        //down right
        memcpy(dst + ((src_horizontal * src_vertical) * 2) + (j * dst_horizontal) + src_horizontal,
               src + (j* src_horizontal) , src_horizontal * 2);
    }
    return;
}
...
int main(int argc, char **argv)
{
    ...
    uint32_t dst_size = ((1920*1080)*2) *3 * 4;
    ...
    for (int i=0;;i++) {
        ...
        //cb
        copy_hd_to_4k(dst_data + (3840*2160), src_data + (1920*1080));

        //cr
        copy_hd_to_4k(dst_data + (3840*2160) * 2, src_data + (1920*1080)*2);
